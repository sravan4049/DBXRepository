...
SELECT is_account_group_member('account users'), is_account_group_member('ADMIN');
...
select group_name, is_member(group_name) from (
  select CONCAT("ANALYST_", country) as group_name from field_demos.uc_acl.customers)
...
CREATE VIEW IF NOT EXISTS field_demos.uc_acl.customer_dynamic_view  AS (
  SELECT * FROM field_demos.uc_acl.customers as customers WHERE is_account_group_member(CONCAT("ANALYST_", country))
);
-- Then grant select access on the view only
GRANT SELECT ON VIEW field_demos.uc_acl.customer_dynamic_view TO `account users`;
...
select * from field_demos.uc_acl.customer_dynamic_view
...
select * from field_demos.uc_acl.analyst_permissions where analyst_email = current_user()

-- COMMAND ----------

-- DBTITLE 1,Let's create the secure view to filter PII information and country based on the analyst permission
CREATE OR REPLACE VIEW field_demos.uc_acl.customer_dynamic_view_gdpr AS (
  SELECT 
  id ,
  creation_date,
  country,
  ...
  FROM 
    field_demos.uc_acl.customers as customers INNER JOIN 
    field_demos.uc_acl.analyst_permissions country  ON country_filter=country
  WHERE 
    country.analyst_email=current_user() 
);
-- Then grant select access on the view only
GRANT SELECT ON VIEW field_demos.uc_acl.customer_dynamic_view_gdpr TO `account users`;
...
-- MAGIC %sql select * from field_demos.uc_acl.customer_dynamic_view_gdpr 
...
UPDATE field_demos.uc_acl.analyst_permissions SET country_filter='USA', gdpr_filter=1 where analyst_email=current_user();

select * from field_demos.uc_acl.customer_dynamic_view_gdpr ;
...
-- For demo only, granting all user ownership in the views so that they can easily update them
ALTER VIEW field_demos.uc_acl.customer_dynamic_view_gdpr OWNER TO `account users`;
ALTER VIEW field_demos.uc_acl.customer_dynamic_view OWNER TO `account users`;
