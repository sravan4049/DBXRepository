...
-- Note: you'll need to be ADMIN to run this command, the catalog has been created for you
CREATE CATALOG IF NOT EXISTS field_demos;
...
CREATE SCHEMA IF NOT EXISTS field_demos.uc_acl;
USE field_demos.uc_acl;

-- COMMAND ----------

-- DBTITLE 1,Let's make sure that all users can use the uc_acl schema for our demo:
GRANT CREATE, USAGE ON SCHEMA field_demos.uc_acl TO `account users`;
...
-- MAGIC If you want to change the owner you can set it as following: ```ALTER TABLE field_demos.uc_acl.customers OWNER TO `account users`;```

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS field_demos.uc_acl.customers (
  id BIGINT,
...
COPY INTO field_demos.uc_acl.customers  FROM '/demo/uc/users' FILEFORMAT=JSON 

-- COMMAND ----------

SELECT * FROM  field_demos.uc_acl.customers
...
GRANT SELECT ON TABLE field_demos.uc_acl.customers TO `account users`;

-- We'll grant an extra MODIFY to our Data Engineer
GRANT SELECT, MODIFY ON TABLE field_demos.uc_acl.customers TO `clement.lacoudre@databricks.com`;

-- COMMAND ----------

SHOW GRANTS ON TABLE field_demos.uc_acl.customers
